...
class ViewsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.original_zendesk_token, settings.ZENDESK_TOKEN = \
            settings.ZENDESK_TOKEN, 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'
    ...
    def testNormalAuthentiction(self):
        ...
        expected = 'http://example.com/access/remoteauth/?name=Alice%20Smith&email=alice%40' \
    ...
    def testAnonymous(self):
        ...
        response = self.client.get(reverse(authorize), {'timestamp': 100}, follow=False)
        self.assertEqual(response.status_code, 302)
        expected = 'http://example.com/access/remoteauth/?name=&email=anona%40' \
    ...
    def testUtfName(self):
        ...
        response = self.client.get(reverse(authorize), {'timestamp': 100}, follow=False)
        self.assertEqual(response.status_code, 302)
        expected = 'http://example.com/access/remoteauth/?name=%CE%94%CE%B5%CE%BD%20%CE%B5%CE%BB' \
